...
static void initRandomDrops(Front *, double **, double *, int *, int *, double, double);
static PARTICLE *initRandomParticles(Front *, int &, double, double);
    ...
    if (string[0] == 'P' || string[0] == 'p')
    {
        *prob_type = PARTICLE_TRACKING;
        ...
        if (string[1] == 'R' || string[1] == 'r')
        {
            ...
            CursorAfterStringOpt(infile, "Enter yes to consider sedimentation:");
            fscanf(infile, "%s ", string);
            (void)printf("%s\n", string);
...
static void readCustomOption(FILE *infile, PARAMS *eqn_params, IF_PARAMS *iFparams)
{
    char string[100];
...
extern void readPhaseParams(Front *front)
{
    FILE *infile;
    char scheme[200];
    int i, num_phases;
    ...
    if (CursorAfterStringOpt(infile, "Choose numerical scheme"))
    {
        ...
        (void)printf("%s\n", scheme);
        if ((scheme[0] == 'E' || scheme[0] == 'e') && (scheme[1] == 'X' || scheme[1] == 'x'))
            eqn_params->num_scheme = UNSPLIT_EXPLICIT;
        else if ((scheme[0] == 'E' || scheme[0] == 'e') && (scheme[1] == 'C' || scheme[1] == 'c'))
            eqn_params->num_scheme = UNSPLIT_EXPLICIT_CIM;
        else if ((scheme[0] == 'I' || scheme[0] == 'i') && (scheme[1] == 'M' || scheme[1] == 'm'))
            eqn_params->num_scheme = UNSPLIT_IMPLICIT;
        else if ((scheme[0] == 'C' || scheme[0] == 'c') && (scheme[1] == 'N' || scheme[1] == 'n'))
            eqn_params->num_scheme = CRANK_NICOLSON;
        else if ((scheme[0] == 'I' || scheme[0] == 'i') && (scheme[1] == 'C' || scheme[1] == 'c'))
...
extern void readWaterDropsParams(Front *front, char *restart_name)
{
    ...
    if (eqn_params->prob_type == PARTICLE_TRACKING)
    {
        ...
        FT_VectorMemoryAlloc((POINTER *)&eqn_params->particle_array, num_drops, sizeof(PARTICLE));
    ...
    case 2:
        intfc_curve_loop(intfc, c)
        {
            if (wave_type(*c) == ICE_PARTICLE_BOUNDARY)
            {
                ...
                total_mass(*c) = drop_dens * PI * 4.0 / 3.0 * radius * radius * radius;
            }
        }
        break;
    case 3:
        intfc_surface_loop(intfc, s)
        {
            if (wave_type(*c) == ICE_PARTICLE_BOUNDARY)
            {
                ...
                total_mass(*s) = drop_dens * PI * 4.0 / 3.0 * radius * radius * radius;
            }
        }
    }
} /* end initWaterDropsParams */

extern void initWaterDrops(Front *front)
{
    char string[100], msg[200];
    char *inname = InName(front);
    FILE *infile = fopen(inname, "r");
    ...
    for (i = 0; i < num_drops; ++i)
    {
        ...
        if (dim == 2)
        {
            FT_MakeEllipticCurve(front, center[i], radii, SOLID_COMP, LIQUID_COMP2, w_type, 1, &curve);
            Gindex(curve) = gindex[i] + 10;
            node_type(curve->start) = CLOSED_NODE;
            body_index(curve) = gindex[i];
            total_mass(curve) = drop_dens * PI * 4.0 / 3.0 * radius[i] * radius[i] * radius[i];
        ...
        {
            SURFACE **s;
            FT_MakeEllipticSurf(front, center[i], radii, SOLID_COMP, LIQUID_COMP2, w_type, 1, &surf);
            Gindex(surf) = gindex[i] + 10;
            body_index(surf) = gindex[i];
            total_mass(Hyper_surf(surf)) = drop_dens * PI * 4.0 / 3.0 * radius[i] * radius[i] * radius[i];
            for (l = 0; l < dim; ++l)
            {
...
static PARTICLE *initRandomParticles(Front *front, int &num_drops, double r_bar, double sigma)
{
    int i, j, count;
    ...
    //printf("tzhang boundary %f %f %f %f\n",L[0],U[0],local_L[0],local_U[0]);
    //printf("tzhang num_drops=%d\n",num_drops);

    FT_VectorMemoryAlloc((POINTER *)&particle_array, num_drops, sizeof(PARTICLE));
    xsubi[0] = 10;
    xsubi[1] = 100;
    xsubi[2] = 1000;
...
static void initRandomDrops(Front *front, double **center, double *radius, int *gindex, int *num_drops, double r_bar,
                            double sigma)
{
        ...
        for (dir = 0; dir < dim; ++dir)
        {
            if (FT_BoundaryType(dir, 0) == PERIODIC_BOUNDARY)
            {
                ...
                for (ii = 0; ii < np; ++ii)
                    for (jj = 0; jj < n; ++jj)
                    {
                        dist = distance_between_positions(pcenter[np + ii], center[jj], dim);
                        if (dist < (radius[jj] + radius[n]))
                            periodic_pair_passed = NO;
                    }
                if (periodic_pair_passed == NO)
                    break;
                np *= 2;
            }
        }
...
extern void read_crt_dirichlet_bdry_data(char *inname, Front *front, F_BASIC_DATA f_basic)
{
    char msg[100], s[100];
    ...
    for (i = 0; i < dim; ++i)
    {
        if (f_basic.boundary[i][0] == DIRICHLET_BOUNDARY)
        {
            ...
            if (rect_boundary_type(front->interf, i, 0) == DIRICHLET_BOUNDARY)
                hs = FT_RectBoundaryHypSurf(front->interf, DIRICHLET_BOUNDARY, i, 0);
            ...
            case 'C':
                ...
                (void)printf("%f\n", state.temperature);
                FT_InsertDirichletBoundary(front, NULL, NULL, NULL, (POINTER)&state, hs, i_surf);
                break;
            default:
        ...
        if (f_basic.boundary[i][1] == DIRICHLET_BOUNDARY)
        {
            ...
            if (rect_boundary_type(front->interf, i, 1) == DIRICHLET_BOUNDARY)
                hs = FT_RectBoundaryHypSurf(front->interf, DIRICHLET_BOUNDARY, i, 1);
