...
static double linear_flux(double wave_speed, double u)
{
    return u;
}

static double linear_dF(double wave_speed, double u)
{
    return 1.0;
}
...
void WENO_FLUX::Weno5_Get_Flux(double *u, /*scalar field*/
                               double *v, /*wave speed*/
                               double *flux, double lambda, int n)
{
    ...
    double a[3][3] = {
        {1.0 / 3.0, -7.0 / 6.0, 11.0 / 6.0}, {-1.0 / 6.0, 5.0 / 6.0, 1.0 / 3.0}, {1.0 / 3.0, 5.0 / 6.0, -1.0 / 6.0}};
     ...
     x[j+k-1+nrad], x[j+k+nrad]} */
    {
        ...
        is[0] = 13.0 / 12.0 * (f[j] - 2.0 * f[j + 1] + f[j + 2]) * (f[j] - 2.0 * f[j + 1] + f[j + 2]) +
                0.25 * (f[j] - 4.0 * f[j + 1] + 3.0 * f[j + 2]) * (f[j] - 4.0 * f[j + 1] + 3.0 * f[j + 2]);
        is[1] = 13.0 / 12.0 * (f[j + 1] - 2.0 * f[j + 2] + f[j + 3]) * (f[j + 1] - 2.0 * f[j + 2] + f[j + 3]) +
                0.25 * (f[j + 1] - f[j + 3]) * (f[j + 1] - f[j + 3]);
        is[2] = 13.0 / 12.0 * (f[j + 2] - 2.0 * f[j + 3] + f[j + 4]) * (f[j + 2] - 2.0 * f[j + 3] + f[j + 4]) +
                0.25 * (3.0 * f[j + 2] - 4.0 * f[j + 3] + f[j + 4]) * (3.0 * f[j + 2] - 4.0 * f[j + 3] + f[j + 4]);
    ...
    {
        /* compute the weights omega[] */
        is[0] = 13.0 / 12.0 * (f[j + 5] - 2.0 * f[j + 4] + f[j + 3]) * (f[j + 5] - 2.0 * f[j + 4] + f[j + 3]) +
                0.25 * (f[j + 5] - 4.0 * f[j + 4] + 3.0 * f[j + 3]) * (f[j + 5] - 4.0 * f[j + 4] + 3.0 * f[j + 3]);
        is[1] = 13.0 / 12.0 * (f[j + 4] - 2.0 * f[j + 3] + f[j + 2]) * (f[j + 4] - 2.0 * f[j + 3] + f[j + 2]) +
                0.25 * (f[j + 4] - f[j + 2]) * (f[j + 4] - f[j + 2]);
        is[2] = 13.0 / 12.0 * (f[j + 3] - 2.0 * f[j + 2] + f[j + 1]) * (f[j + 3] - 2.0 * f[j + 2] + f[j + 1]) +
                0.25 * (3.0 * f[j + 3] - 4.0 * f[j + 2] + f[j + 1]) * (3.0 * f[j + 3] - 4.0 * f[j + 2] + f[j + 1]);
...
void WENO_FLUX::printField(double *var, const char *var_name)
{
    ...
    char filename[200];
    int ic, i, j;
    sprintf(filename, "%s/%s-%d-nd%d", OutName(front), var_name, front->step, pp_mynode());
...
void WENO_FLUX::addFluxInDirection(int dir, double **U, /*wave speed*/
...
void WENO_FLUX::addFluxInDirection1d(int dir, double **U, double *T, double *adv)
{
    printf("1d was not implemented!\n");
    return;
}

void WENO_FLUX::addFluxInDirection2d(int dir, double **U, double *T, double *adv)
{
    static double *vU;
    static double *vT;
    static double *vflux;
...
void WENO_FLUX::addFluxInDirection3d(int dir, double **U, double *T, double *adv)
{
    static double *vU;
    static double *vT;
    static double *vflux;
...
void WENO_FLUX::appendGhostBuffer(double *T, double *vT, int n, int *icoords, int idir, int nb)
{
    int i, index;
    HYPER_SURF *hs;
    ...
    case 0:
        for (i = 1; i <= nrad; i++)
        {
            ...
            is_crxing = (*findStateAtCrossing)(front, ic, ldir[idir], comp, &state, &hs, crx_coords);
    ...
    case 1:
        ...
        {
            ic[idir] = icoords[idir] + i - 1;
            is_crxing = (*findStateAtCrossing)(front, ic, rdir[idir], comp, &state, &hs, crx_coords);
            if (is_crxing == NO_PDE_BOUNDARY)
            {
                ic[idir]++;
                index = d_index(ic, top_gmax, dim);
