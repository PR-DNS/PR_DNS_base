...
#define SOLID_COMP 1
#define alternate_comp(comp) (comp) == LIQUID_COMP2 ? SOLID_COMP : LIQUID_COMP2
#ifndef __CUDA__
// #define __CUDA__
#endif
...
struct _PARTICLE
{
    ...
    double center[MAXD];
    double vel[MAXD];
    int ic[MAXD]; //tzhang for collision
    double R0;
    double rho;
...
class VCARTESIAN
{
  ...
  public:
    ...
    void computeVaporSource_CUDA(int num_drops, PARAMS *eqn_params, int gmax0, int gmax1, double rho_0, double a3,
                                 double lcp);
...
class WENO_FLUX
{
  ...
  public:
    ...
    void computeAdvection();
    int (*findStateAtCrossing)(Front *, int *, GRID_DIRECTION, int, POINTER *, HYPER_SURF **, double *);
...
extern void melt_flowThroughBoundaryState(double *, HYPER_SURF *, Front *, POINTER, POINTER);
