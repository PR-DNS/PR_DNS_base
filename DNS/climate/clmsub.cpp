...
extern void melt_flowThroughBoundaryState(double *p0, HYPER_SURF *hs, Front *front, POINTER params, POINTER state)
{
    Tan_stencil **tsten;
    Nor_stencil *nsten;
    ...
    {
        double vtmp;
        FT_IntrpStateVarAtCoords(front, comp, nsten->pts[1], iF_field->vel[i], getStateVel[i], &vtmp, &sts[0]->vel[i]);
        u[2] += vtmp * dir[i];
        v[2][i] = vtmp * (1.0 - dir[i]);
    }
    if (dim == 2)
    {
        FT_IntrpStateVarAtCoords(front, comp, nsten->pts[1], iF_field->vort, getStateVort, &vort[2], &sts[1]->vort);
    }
    FT_IntrpStateVarAtCoords(front, comp, nsten->pts[1], iF_field->pres, getStatePres, &pres[2], &sts[1]->pres);
    FT_IntrpStateVarAtCoords(front, comp, nsten->pts[1], pH_field->temperature, getStateTemperature, &temp[2],
                             &sts[1]->temperature);
...
static void vFourier_state(COMPONENT *c, double *coords, PHASE_FIELD *field, int index, int dim, PARAMS *eqn_params)
{
    double qs, qe, qmax;
    qs = eqn_params->qs;
    qe = eqn_params->qe;
...
static void lr_state(COMPONENT *c, double *coords, PHASE_FIELD *field, int index, int dim, PARAMS *eqn_params)
{
    ...
    {
        if (coords[0] > x0 - width * frac * 0.5 && coords[0] < x0 + width * frac * 0.5)
            field->vapor[index] = qmax;
        else
            field->vapor[index] = qe;
    }
    else
    {
        ...
        field->vapor[index] = (qmax - qe) * exp(-A * pow(coords[0] / width - 0.5, 8.0)) + qe;
    }
}

static void tb_state(COMPONENT *c, double *coords, PHASE_FIELD *field, int index, int dim, PARAMS *eqn_params)
{
    double qs, qe, qmax;
    double x0, A;
    ...
    if (eqn_params->frac > 0 && eqn_params->frac < 1)
    {
        if (coords[dim - 1] > x0 - width * frac * 0.5 && coords[dim - 1] < x0 + width * frac * 0.5)
            field->vapor[index] = qmax;
        else
            field->vapor[index] = qe;
    }
    ...
    {
        A = pow(0.512, 8.0) * 1.0e6;
        field->vapor[index] = (qmax - qe) * exp(-A * pow(coords[dim - 1] - x0, 6)) + qe;
    }
}

static void sine_state(COMPONENT *c, double *coords, PHASE_FIELD *field, int index, int dim, PARAMS *eqn_params)
{
    field->vapor[index] = sin(PI * (coords[0] + coords[1]));
}

void init_vapor_state_func(Front *front, VCARTESIAN *vcartesian)
{
    PARAMS *eqn_params = (PARAMS *)front->extra2;
...
void init_temp_state_func(Front *front, VCARTESIAN *vcartesian)
{
    PARAMS *eqn_params = (PARAMS *)front->extra2;
...
void init_fluid_state_func(Front *front, Incompress_Solver_Smooth_Basis *l_cartesian)
{
    PARAMS *eqn_params = (PARAMS *)front->extra2;
...
static void Rogallo_state(COMPONENT comp, int *N, double *vel_x, double *vel_y, double *vel_z, RECT_GRID *gr,
                          IF_PARAMS *iFparams)
{
    if (debugging("trace"))
            ...
            for (j = 0; j < gmax[1]; j++)
                for (i = 0; i < gmax[0]; i++)
                {
                    ...
                    if (w[0] == 0 && w[1] == 0)
                        continue;
                    U[index][0] =
                        (alpha[0] * wn * w[1] + beta[0] * w[0] * w[2]) / (wn * sqrt(w[0] * w[0] + w[1] * w[1]));
                    U[index][1] =
                        (alpha[1] * wn * w[1] + beta[1] * w[0] * w[2]) / (wn * sqrt(w[0] * w[0] + w[1] * w[1]));
                    V[index][0] =
                        (beta[0] * w[1] * w[2] - alpha[0] * wn * w[0]) / (wn * sqrt(w[0] * w[0] + w[1] * w[1]));
                    V[index][1] =
                        (beta[1] * w[1] * w[2] - alpha[0] * wn * w[0]) / (wn * sqrt(w[0] * w[0] + w[1] * w[1]));
...
static void Fourier_state(COMPONENT comp, int *N, double *vel_x, double *vel_y, double *vel_z, RECT_GRID *gr,
                          IF_PARAMS *iFparams)
{
    if (debugging("trace"))
        printf("Entering Fourier_state()\n");
...
static void ABC_state(COMPONENT comp, double *coords, IF_FIELD *field, int index, int dim, IF_PARAMS *iFparams)
{
    int i;
...
static void Taylor_Green_state(COMPONENT comp, double *coords, IF_FIELD *field, int index, int dim, IF_PARAMS *iFparams)
{
    int i;
    double tcoords[MAXD], a[MAXD] = {2 * PI, 2 * PI, 2 * PI}, A[MAXD] = {1., -1., 0.};
    double **vel = field->vel;
...
static void rand_state(COMPONENT comp, double *coords, IF_FIELD *field, int index, int dim, IF_PARAMS *iFparams)
{
    short unsigned int seed[3] = {time(NULL) - index, time(NULL), time(NULL) + index};
...
static void zero_state(COMPONENT comp, double *coords, IF_FIELD *field, int index, int dim, IF_PARAMS *iFparams)
{
    int i;
    for (i = 0; i < dim; ++i)
...
static double intrp_between(double x1, double x2, double x, double y1, double y2)
{
    double y;
    if (x1 == x2)
        return y1;
...
LOCAL PARTICLE *cut_buf_particle(PARTICLE *particle_array, Front *front, int &buf_size, int dir, int side)
{
    int i, j;
    PARAMS *eqn_params = (PARAMS *)front->extra2;
            ...
            (side == 1 && particle_array[i].center[dir] > ZU[dir]))
        {
            ...
            if (side == 0 && particle_array[i].center[dir] < GL[dir])
                particle_array[i].center[dir] =
                    GU[dir] + fmod(particle_array[i].center[dir] - GL[dir], GU[dir] - GL[dir]);
            if (side == 1 && particle_array[i].center[dir] > GU[dir])
                particle_array[i].center[dir] =
                    GL[dir] + fmod(particle_array[i].center[dir] - GU[dir], GU[dir] - GL[dir]);
...
LOCAL void send_particle(PARTICLE *buf_particle, int buf_size, int dst_id)
{
    ...
    pp_send(array_id(dst_id), (POINTER)buf_particle, sizeof(PARTICLE) * buf_size, dst_id);
    return;
}
...
LOCAL void merge_particle(PARTICLE **particle_array, PARTICLE *adj_particle, int adj_size, PARAMS *params)
{
    static int max_size = 0;
...
private:
    ...
    int hash(int x, int y, int z) const {
        const int p1 = 10000019, p2 = 19349663, p3 = 83492791;
        return (x * p1) ^ (y * p2) ^ (z * p3);
    }
public:
    SpatialHashGrid(int num_drops) {
        total_drops = num_drops;
    }

    void update(PARTICLE *particle_array){
        int i;
        int x,y,z;
        for(i = 0; i < total_drops; i++){
            x = particle_array[i].ic[0];
            y = particle_array[i].ic[1];
            z = particle_array[i].ic[2];
            grid[hash(x,y,z)].push_back(&particle_array[i]);
        }
    }
...
extern float solveCollisionTime(float a, float b, float c){
    float D = b * b - 4 * a * c;
...
extern float checkCollision(const PARTICLE a, const PARTICLE* b){
    
    float dvx = a.vel[0] - b->vel[0];
    float dvy = a.vel[1] - b->vel[1];
    float dvz = a.vel[2] - b->vel[2];
    ...
    float B = 2 * (dvx * dx + dvy * dy + dvz * dz);
    float C = dx * dx + dy * dy + dz * dz - rSum * rSum;
    
    float t = solveCollisionTime(A,B,C);
    if (t != INFINITY) return 1;

    return 0;
}

void mergeDroplets(PARTICLE* a,PARTICLE* b){
    double r1Vol, r2Vol, rNewVol;
    double px, py, pz;
    
    r1Vol = a->radius * a->radius * a->radius;
    r2Vol = b->radius * b->radius * b->radius;
    rNewVol = r1Vol + r2Vol;
    ...
    a->vel[1] = (a->vel[1] * r1Vol + b->vel[1] * r2Vol)/rNewVol;
    a->vel[2] = (a->vel[2] * r1Vol + b->vel[2] * r2Vol)/rNewVol;

    //new radius
    a->radius = std::cbrt(rNewVol);
    b->radius = 0;
}

extern void collision_coalescence(Front *fr)
{
    int i;
    double dx, dy, dz;
    double factor;
    ...
    SpatialHashGrid SHgrid(eqn_params->num_drops);

    //update the spatialhash grid 
    SHgrid.update(particle_array);
   ...
   for (i = 0; i < eqn_params->num_drops; i++){
       ...
       if (dist_flow == 1){
           for(auto* pp: neighbors){
               dx = p1.center[0] - pp->center[0];
               dy = p1.center[1] - pp->center[1];
               dz = p1.center[2] - pp->center[2];
    
               double dist = std::sqrt(dx * dx + dy * dy + dz *dz);
    
               if (dist != 0){
                   double cross_x = pp->center[1] * dz - pp->center[2] * dy;
                   double cross_y = pp->center[2] * dx - pp->center[0] * dz;
                   double cross_z = pp->center[0] * dy - pp->center[1] * dx;
    
                   factor = k/((dist+epsilon) * (dist+epsilon) * (dist+epsilon));  
                   if (factor >= 1.0) factor = 0.5;               
                   particle_array[i].vel[0] += cross_x * factor;
                   particle_array[i].vel[1] += cross_y * factor;
                   particle_array[i].vel[2] += cross_z * factor;
               }  
           }
       }
       
       for (auto* p2: neighbors){
           if(p1.Gindex >= p2->Gindex) continue;
...
{
    printf("tzhang ParticlePropagate() : Entering ParticlePropage()\n");
    if (debugging("trace"))
    ...
    printf("tzhang total num_drops %d\n",eqn_params->num_drops);
    ...
    for (i = 0; i < eqn_params->num_drops; i++)
    {
        ...
        for (j = 0; j < dim; j++)
        {
            ...
            FT_IntrpStateVarAtCoords(fr, LIQUID_COMP, center, eqn_params->field->temperature, getStateTemperature,
                                     &temp, &temp);
        }
        ...
        for (j = 0; j < dim; ++j)
        {
            ...
            if (eqn_params->if_sedimentation)
            {
                center[j] += tau_p * (1 - exp(-dt / tau_p)) * cvel[j] +
                             (dt - tau_p + tau_p * exp(-dt / tau_p)) * (u[j] + gravity[j] * tau_p);
                cvel[j] = exp(-dt / tau_p) * cvel[j] + (1 - exp(-dt / tau_p)) * (u[j] + gravity[j] * tau_p);
            }
            else
            {
                center[j] +=
                    tau_p * (1 - exp(-dt / tau_p)) * cvel[j] + (dt - tau_p + tau_p * exp(-dt / tau_p)) * (u[j]);
                cvel[j] = exp(-dt / tau_p) * cvel[j] + (1 - exp(-dt / tau_p)) * (u[j]);
            }
        ...
        if (debugging("single_particle"))
        {
            ...
            printf("Flow_vel[%f,%f,%f]\nc_vel[%f,%f,%f]\n", vel[0][index], vel[1][index], vel[2][index], cvel[0],
                   cvel[1], cvel[2]);
            printf("Response time = %f\n", tau_p);
        }
    } //end loop for all particles
    
    //add the collision_coalescence by TZ
    
    printf("tzhang before collision_coalescence\n");
    //eqn_params->particle_array = particle_array;
    collision_coalescence(fr);
    printf("tzhang after collision_coalescence\n");
...
extern void setParticleGroupIndex(PARTICLE *particle_array, int size, int dim, int *Nclip, double *L, double *U)
{
...
extern void printDropletsStates(Front *front, char *outname)
{
    ...
    sprintf(filename, "%s/state.ts%s", outname, right_flush(front->step, 7));
...
extern void vtk_plot_sample_traj(Front *front)
{
    ...
    sprintf(fname, "%s/vtk/vtk.ts%s/", outname, right_flush(front->step, 7));
...
extern double *ComputePDF(double *array, int size, double &bin_size, int num_bins, double &var_min, double &var_max)
{
